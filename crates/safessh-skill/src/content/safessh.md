...
- `safessh <project> exec -- <command>` — run a command on the remote host.
  The command is parsed and matched against the project's policy. If allowed
  it runs; if it requires approval, safessh prints a `BLOCKED:` token; if it
  is denied outright, safessh prints a `BLOCKED:` reason and exits non-zero.
- `safessh project list` — list configured projects.
- `safessh project add <name>` — interactively register a new project (the
  user does this; do not run it on their behalf without being asked).
- `safessh approve <token>` — approve a previously-blocked command using the
  token printed by `exec`. The user typically runs this; only run it if the
