# Release stamps

## v0.6.0 (prepared 2026-05-03)
- Bumped on branch `feature/v0.5-v0.6` after the v0.5.0 stamp.
- Follow the canonical v0.4.0 template below, substituting `v0.6.0`.

## v0.5.0 (prepared 2026-05-03)
- Bumped on branch `feature/v0.5-v0.6`.
- Follow the canonical v0.4.0 template below, substituting `v0.5.0`.

---

# Release checklist for v0.4.0

safessh uses cargo-dist for releases. This is the manual sequence to actually
ship v0.4.0 to users. The same template was used for v0.2.0 / v0.3.0 — bump the
version strings throughout for future releases.

## Prerequisites

The Homebrew tap repo (`sanif/homebrew-tap`) and the safessh repo (`sanif/safessh`)
already exist from prior releases. The `dist-workspace.toml` `tap` field and
`release.yml` repository reference both point at `sanif/homebrew-tap`.

v0.4.0 sits on top of v0.3.0; if `feature/v0.3.0` has not yet merged to
`develop`, merge it first (`git merge --no-ff feature/v0.3.0` from `develop`)
so the v0.4 merge isn't carrying both milestones at once.

## Step 1: Pre-release verification

Merge `feature/v0.4.0` into `develop`, then on `develop`:

- `cargo build --workspace`
- `cargo test --workspace`
- `cargo test --workspace --features integration` (requires Docker; Linux only)
- `cargo fmt --all --check`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `dist plan` shows 4 targets + 2 installers
- Manual sanity: `cargo run --bin safessh -- tui` opens the TUI on a real
  terminal; `q` quits cleanly without leaving the terminal in raw mode.
- Manual sanity: piped invocation (`safessh tui </dev/null`) exits 1 with
  `safessh: error: tui requires a TTY`.
- Manual sanity: `safessh prod forward 15432:localhost:22` against a project
  that allows `network:tunnel` returns immediately with the
  `tunnel open id=…` line; `safessh tunnels list` shows the row;
  `safessh tunnels close <id>` returns `closed tunnel <id>` and the row
  disappears from the next `tunnels list`.

## Step 2: Merge develop → main

```sh
git checkout main
git merge --no-ff develop
git push origin main
```

## Step 3: Tag and push (rc first)

```sh
git tag -a v0.4.0-rc.1 -m "Release candidate 1 for v0.4.0"
git push origin v0.4.0-rc.1
```

GitHub Actions runs the release workflow. Verify:

- GitHub Release created with 4 binaries (macOS arm64/x64, Linux x64/arm64).
- Checksum file present.
- `installer.sh` URL resolves.
- Homebrew formula updated in `sanif/homebrew-tap`.

## Step 4: Clean-machine verification

On a clean macOS arm64 (or new Mac VM):

- `brew upgrade sanif/tap/safessh` (or `brew install sanif/tap/safessh` on a
  fresh box).
- `safessh --version` prints `safessh 0.4.0-rc.1`.
- `safessh project add demo --alias localhost`
- `safessh demo forward 15432:localhost:22` returns immediately with the
  tunnel-open line.
- `safessh tunnels list` shows the row; `safessh tunnels close <id>` succeeds.
- `safessh tui` opens the TUI; cycle Tab through the four screens; `?`
  shows the help overlay; `q` quits cleanly.

On a clean Linux x86_64 (a fresh Docker container is fine — the container
just needs a TTY allocated, e.g. `docker run -it ubuntu`):

- `curl -fsSL <installer-url> | sh`
- Same verification commands as above.

## Step 5: Promote to v0.4.0

If rc.1 is clean:

```sh
git tag -a v0.4.0 -m "v0.4.0 release"
git push origin v0.4.0
```

If rc.1 has issues, fix them on `develop`, fast-forward `main`, and tag
`v0.4.0-rc.2`. Repeat until a clean rc.

## Step 6: Update CHANGELOG

- Move the date in `CHANGELOG.md` to the actual release date if it changed
  from the placeholder.
- Add an empty `[Unreleased]` section above the `[0.4.0]` block (already
  present from feature work — leave it).
- Commit: `chore: release v0.4.0`.

## Step 7: Back-merge into develop

```sh
git checkout develop
git merge --no-ff main
git push origin develop
```

This keeps `develop` ahead of (or equal to) `main` for the next milestone.
